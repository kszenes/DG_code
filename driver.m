...
%     h0=1e7; h1=1e3; Cx=a+(b-a)*1/2; Cy=c+(d-c)*1/2; sigma=(a+b)/20;
    ...
    % u0(:,:,1)=h0_fun(pts2d_phi(1:n_qp,:),pts2d_phi(n_qp+1:2*n_qp,:)).*fact_int;
    % u0(:,:,2)=u0(:,:,1).*v0x_fun(pts2d_phi(1:n_qp,:),pts2d_phi(n_qp+1:2*n_qp,:));
    % u0(:,:,3)=u0(:,:,1).*v0y_fun(pts2d_phi(1:n_qp,:),pts2d_phi(n_qp+1:2*n_qp,:));
